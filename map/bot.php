...
if (empty($args['mcc'])) {
    echo 'MCC is required';
    exit();
}
if (empty($args['mnc'])) {
    echo 'MNC is required';
    exit();
}
if (empty($args['mobile_type'])) {
    $args['mobile_type'] = 1;
}
if (empty($args['lac'])) {
    echo 'LAC is required';
    exit();
}
if (empty($args['cellid'])) {
    echo 'CELLID is required';
    exit();
}
if (empty($args['sid'])) {
    $args['sid'] = '';
...
if (!$dataExtracted['data']) {
    ...
    echo 'lat : ' .
        $dataExtracted['data']['lat'] .
        "\nlon : " .
        $dataExtracted['data']['lon'] .
        "\nAccuracy : " .
        $dataExtracted['data']['range'];
    echo "\nGoogle Maps : https://www.google.com/maps/search/?api=1&query=" .
        $dataExtracted['data']['lat'] .
        ', ' .
        $dataExtracted['data']['lon'];
        var_dump($dataExtracted);
}
